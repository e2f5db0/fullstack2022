...
const dailyExerciseHours = process.argv.slice(2, process.argv.length)
const dailyExerciseHoursNumber = dailyExerciseHours.map(t => Number(t))
...
const calculateExercises = (dailyExerciseHours: Array<number>, target: number): Result => {
    ...
    let sum = 0;
    for (let t of dailyExerciseHours) {
        sum += t
    }
...
try {
    console.log(calculateExercises(dailyExerciseHoursNumber, target))
} catch (error: unknown) {
